...
class LokerController extends Controller
{
    ...
    public function ApplyLokerStore(Request $request)
    {
        ...
        if ($request->hasFile('pengalaman')) {
            $file = $request->file('pengalaman');
            $pengalamanName =  $id_user . '- pengalaman'  . '.' . $file->getClientOriginalExtension(); // Menggunakan $file
            $file->move(public_path('uploads/' . $id_loker . '/pengalaman'), $pengalamanName); // Menggunakan $file
        }
        if ($request->hasFile('ijazah')) {
            $file = $request->file('ijazah');
            $ijazahName =  $id_user . '- ijazah'  . '.' . $file->getClientOriginalExtension(); // Menggunakan $file
            $file->move(public_path('uploads/' . $id_loker . '/ijazah'), $ijazahName); // Menggunakan $file
        }
        if ($request->hasFile('lamaran')) {
            $file = $request->file('lamaran');
            $lamaranName =  $id_user . '- lamaran'  . '.' . $file->getClientOriginalExtension(); // Menggunakan $file
            $file->move(public_path('uploads/' . $id_loker . '/lamaran'), $lamaranName); // Menggunakan $file
        }
    ...
    public function addLokerstore(Request $request)
    {
        ...
        $lamaran = $request->input('lamaran', 0); // Mengambil nilai dari input checkbox7 atau kembalikan 0 jika tidak ada
        $ijazah = $request->input('ijazah', 0); // Mengambil nilai dari input checkbox7 atau kembalikan 0 jika tidak ada
        $pengalaman = $request->input('pengalaman', 0); // Mengambil nilai dari input checkbox7 atau kembalikan 0 jika tidak ada
    ...
    public function showListbyApply()
    {
        $tanggal = "1995-10-10";
        $apply = Apply::all();
        return view('loker.listapplybyloker', compact('apply', 'tanggal'));
    }
    ...
    public function compressAndDownload(Request $request, $id)
    {
        ...
        if ($zip->open($zipFilePath, ZipArchive::CREATE) === true) {
            ...
            $kkPath = public_path("uploads/{$loker->id_loker}/kartu_keluarga/{$loker->kartu_keluarga}");
            $ktpPath = public_path("uploads/{$loker->id_loker}/ktp/{$loker->ktp}");
            $ijazahPath = public_path("uploads/{$loker->id_loker}/ijazah/{$loker->ijazah}");
            $pengalamanPath = public_path("uploads/{$loker->id_loker}/pengalaman/{$loker->pengalaman}");
            $lamaranPath = public_path("uploads/{$loker->id_loker}/lamaran/{$loker->lamaran}");
            ...
            if (file_exists($ijazahPath)) {
                $zip->addFile($ijazahPath, $loker->ijazah);
                $filesAdded[] = $ijazahPath;
            }
            if (file_exists($pengalamanPath)) {
                $zip->addFile($pengalamanPath, $loker->pengalaman);
                $filesAdded[] = $pengalamanPath;
            }
            if (file_exists($lamaranPath)) {
                $zip->addFile($lamaranPath, $loker->lamaran);
                $filesAdded[] = $lamaranPath;
            }
